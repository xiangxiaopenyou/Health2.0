...
@interface AttentionViewController ()<UITableViewDataSource,UITableViewDelegate,EGORefreshTableHeaderDelegate>{
...
- (void)viewDidLoad {
    ...
    [self.tableview setBackgroundColor:TABLEVIEW_BACKGROUNDCOLOR];
...
- (void)refresh{
    ...
    if (self.flagIndex == 2) {
        url = [NSString stringWithFormat:@"%@%@",URL_BASE,URL_FRIEND_ATTENTION];
        ...
        url = [NSString stringWithFormat:@"%@%@",URL_BASE,URL_MY_ATTENTION];
...
- (void)loadMore{
    ...
    if (dataArray.count == 0 || dataArray == nil) {
        ...
        //[dictionary setValue:entity.time forKey:@"time"];
...
//        fanscell.backgroundColor = [UIColor clearColor];
        ...
        }else if ([myfan.flag integerValue] == 4){
            [fanscell.btnFocus setBackgroundImage:[UIImage imageNamed:@"havefocused"] forState:UIControlStateNormal];
        }else{
        ...
        [fanscell clickAttention:^(NSInteger index) {
            ...
            if ([myfan.flag integerValue] == 2 ) {
                ...
                myfan.flag = @"1";
            }else if ([myfan.flag integerValue] == 3){
