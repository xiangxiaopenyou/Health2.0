...
@interface ClockinDetailViewController ()<UITableViewDataSource, UITableViewDelegate, UITextViewDelegate, UIAlertViewDelegate, TrendCommentsCellDelegate , UIActionSheetDelegate, EGORefreshTableHeaderDelegate, UIScrollViewDelegate, ClockinDetailCellDelegate>{
    
    UITableView *detailTableView;
    
    ClockinDetailTableViewCell *detailCell;
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        detailCell = [[ClockinDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:DetailCell withDic:detailDic];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        detailCell = (ClockinDetailTableViewCell*)[self tableView:tableView cellForRowAtIndexPath:indexPath];
...
#pragma mark - ClockinDetailCellDelegate
- (void)clickHead{
...
- (void)clickLikeMember{
    TrendLikeMemberViewController *controller = [[TrendLikeMemberViewController alloc] init];
    controller.trendID = [detailDic objectForKey:@"id"];
...
- (void)clickCourse {
    NSArray *club = [[detailDic objectForKey:@"coursetitle"] componentsSeparatedByString:@";"];
    NSArray *tempArray = [[club objectAtIndex:0] componentsSeparatedByString:@":"];
    ClubDetailViewController *controller = [[ClubDetailViewController alloc] init];
    controller.clubId = [tempArray objectAtIndex:1];
    controller.clubNameString = [tempArray objectAtIndex:0];
    [self.navigationController pushViewController:controller animated:YES];
    ...
    if (buttonIndex == 1) {
        ...
        [TrendRequest deleteTrendWith:deleteDic success:^(id response) {
            if ([[response objectForKey:@"state"] integerValue] == 1000) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteclockinsuccess" object:@YES];
            }
            else{
                NSLog(@"删除打卡失败");
...
#pragma mark - TrendCommentCellDelegate
