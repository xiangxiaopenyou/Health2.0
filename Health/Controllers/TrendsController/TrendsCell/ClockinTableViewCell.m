...
@implementation ClockinTableViewCell
@synthesize headImage, nicknameLabel, timeLabel, deleteButton, reportButton, contentLabel, courseLabel, positionLabel, clockinDaysLabel, commentButton, commentNumberLabel, likeButton, likeNumberLabel, pictureView, privateView, weightLabel, sportsEnergyLabel, sportsLabel, foodEnergyLabel, foodLabel, likeImage;
@synthesize height;
...
//            [clockinView addSubview:foodEnergyLabel];
        ...
        UIImageView *commentImage = [[UIImageView alloc] initWithFrame:CGRectMake(20, 7.5, 15, 15)];
        commentImage.image = [UIImage imageNamed:@"trend_comment.png"];
        [commentButton addSubview:commentImage];
        
        commentNumberLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 5, 50, 20)];
        commentNumberLabel.text = [NSString stringWithFormat:@"评论 %@", [dic objectForKey:@"commentnumber"]];
        ...
        likeImage = [[UIImageView alloc] initWithFrame:CGRectMake(20, 7.5, 15, 15)];
        [likeButton addSubview:likeImage];
        
        likeNumberLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 5, 50, 20)];
        ...
        likeNmuber = [[dic objectForKey:@"likenumber"] integerValue];
        
        if ([[dic objectForKey:@"isfavorite"] integerValue] == 2) {
            isLiked = NO;
            likeImage.image = [UIImage imageNamed:@"trend_like.png"];
            likeNumberLabel.text = [NSString stringWithFormat:@"点赞 %ld", (long)likeNmuber];
        }
        else {
            isLiked = YES;
            likeImage.image = [UIImage imageNamed:@"trend_like_pressed.png"];
            likeNumberLabel.text = [NSString stringWithFormat:@"已赞 %ld", (long)likeNmuber];
        }
...
- (void)headClicked:(UITapGestureRecognizer*)gesture{
    NSLog(@"点击了头像");
    [self.delegate clickClockinUserHead:[self.clockinDic objectForKey:@"userid"]];
}
- (void)nicknameClick{
    NSLog(@"点击了昵称");
    [self.delegate clickClockinUserNickname:[self.clockinDic objectForKey:@"userid"]];
}
- (void)deleteClick{
    NSLog(@"点击了删除");
    //[self.delegate clickDelete:singleTrend.trendid];
    [self.delegate clickClockinDelete:[self.clockinDic objectForKey:@"id"]];
}
- (void)reportClick{
    NSLog(@"点击了投诉");
    //[self.delegate clickReport:singleTrend.trendid];
    [self.delegate clickClockinReport:[self.clockinDic objectForKey:@"id"]];
}
...
- (void)courseClick{
    NSLog(@"点击了训练营");
    NSArray *course = [[self.clockinDic objectForKey:@"coursetitle"] componentsSeparatedByString:@";"];
    NSString *courseString = [course objectAtIndex:0];
    courseArray = [courseString componentsSeparatedByString:@":"];
    [self.delegate clickClockinCourse:[courseArray objectAtIndex:1] withTitle:[courseArray objectAtIndex:0]];
}
- (void)commentButtonClick{
    [self.delegate clickClockinComment:[self.clockinDic objectForKey:@"id"]];
}
