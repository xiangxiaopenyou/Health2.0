//
//  WriteClubDiscussionViewController.m
//  Health
//
...
@interface WriteClubDiscussionViewController ()<UITextFieldDelegate, UITextViewDelegate,UIActionSheetDelegate>{
    UILabel *tipLabel;
    BOOL isImage;
    ...
    NSInteger discussoverhead;

}

@end

@implementation WriteClubDiscussionViewController
...
- (void)viewDidLoad {
    ...
    //提示
    tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(94, NAVIGATIONBAR_HEIGHT + 71, 100, 20)];
    tipLabel.text = @"输入正文";
...
//        putTopButton.frame = CGRectMake(SCREEN_WIDTH - 85, 270 + NAVIGATIONBAR_HEIGHT, 75, 20);
        ...
        if (titleTextField.text.length > 30) {
            ...
            if ([Util isEmpty:contentTextView.text]){
                ...
                if (contentTextView.text.length > 2000) {
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"正文不要超过2000字哦~" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                    [alert show];
                }
                else{
...
//                    [JDStatusBarNotification showActivityIndicator:YES indicatorStyle:UIActivityIndicatorViewStyleGray];
                    ...
                    [dic setObject:self.clubId forKeyedSubscript:@"clubid"];
                    [dic setObject:self.clubOwnerId forKeyedSubscript:@"clubuserid"];
                    [dic setObject:titleTextField.text forKey:@"clubpoststitle"];
                    [dic setObject:contentTextView.text forKey:@"clubposcontent"];
                    if (discussoverhead == 1) {
                    ...
                    if (isImage) {
                        ...
                        [RTUploadImageNetWork postMulti:nil imageparams:trendImage.image success:^(id response) {
                            if (![Util isEmpty:response]) {
                                ...
                                [dic setObject:imageString forKey:@"clubpostspicture"];
                                [ClubRequest writeClubDisussionWith:dic success:^(id response) {
