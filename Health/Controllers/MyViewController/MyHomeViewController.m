...
@interface MyHomeViewController ()<EGORefreshTableHeaderDelegate, MineInfoCellDelegate, CLWeeklyCalendarViewDelegate, UIAlertViewDelegate>{
    NSInteger flagIndex;
    ...
    UILabel *tipNeedsConsumeLabel;
    
    UIView *editNowWeightView;
    UIView *editTargetWeightView;
    
    UIPickerView *nowPicker;
    UIPickerView *targetPicker;
    
    UIButton *finishNowWeightButton;
    UIButton *finishTargetWeightButton;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteSuccess) name:@"ShowDelete" object:nil];
...
//    [self getNotReadNotification];
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"firstHomePage"]) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"firstHomePage"];
        [self setupTipView];
    }
}
...
- (void)addViewForTable{
    viewForTable = [[UIView alloc] initWithFrame:CGRectMake(0 - SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT, SCREEN_WIDTH*2, SCREEN_HEIGHT - NAVIGATIONBAR_HEIGHT - TABBAR_HEIGHT)];
    ...
    myTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, viewForTable.frame.size.height)];
    myTableView.delegate = self;
    myTableView.dataSource = self;
    myTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    myTableView.backgroundColor = CLEAR_COLOR;
    myTableView.showsVerticalScrollIndicator = NO;
    [viewForTable addSubview:myTableView];
    if (_refresh == nil) {
        _refresh = [[EGORefreshTableHeaderView alloc] initWithFrame:CGRectMake(0, 0 - myTableView.bounds.size.height, myTableView.frame.size.width, myTableView.bounds.size.height)];
        _refresh.delegate = self;
        _refresh.backgroundColor = [UIColor clearColor];
        [myTableView addSubview:_refresh];
    }
...
//    UIButton *bodyHealthButton = [UIButton buttonWithType:UIButtonTypeCustom];
//    bodyHealthButton.frame = CGRectMake(SCREEN_WIDTH - 60, 10, 50, 30);
//    [bodyHealthButton  setTitle:@"身体状况" forState:UIControlStateNormal];
//    [bodyHealthButton setTitleColor:MAIN_COLOR_YELLOW forState:UIControlStateNormal];
//    [bodyHealthButton addTarget:self action:@selector(bodyHealthClick) forControlEvents:UIControlEventTouchUpInside];
//    bodyHealthButton.titleLabel.font = SMALLFONT_14;
//    [headerView addSubview:bodyHealthButton];
    ...
    reducedWeightLabel.font = [UIFont boldSystemFontOfSize:40];
    ...
    helpButton.frame = CGRectMake(60, 2, 16, 16);
    [helpButton setImage:[UIImage imageNamed:@"help_energy"] forState:UIControlStateNormal];
    [helpButton addTarget:self action:@selector(helpClick) forControlEvents:UIControlEventTouchUpInside];
    [todayEnergyView addSubview:helpButton];
    ...
    UILabel *label3 = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 110, 25, 9, 9)];
...
- (void)setupTipView{
    ...
    UIImageView *tipImage1 = [[UIImageView alloc] initWithFrame:CGRectMake(34, SCREEN_WIDTH/2 + 114 + NAVIGATIONBAR_HEIGHT, 239, 94)];
    tipImage1.image = [UIImage imageNamed:@"tip05"];
    [tipView addSubview:tipImage1];
    
    UIImageView *tipImage2 = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 236, SCREEN_WIDTH/2 - 65 + NAVIGATIONBAR_HEIGHT, 176, 118)];
    tipImage2.image = [UIImage imageNamed:@"tip06"];
    [tipView addSubview:tipImage2];
    
    UIButton *knownButton =[UIButton buttonWithType:UIButtonTypeCustom];
    knownButton.frame = CGRectMake(SCREEN_WIDTH - 102, SCREEN_WIDTH/2 + 144 + NAVIGATIONBAR_HEIGHT, 70, 30);
    ...
    tipNeedsConsumeLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 98, NAVIGATIONBAR_HEIGHT + 53 + SCREEN_WIDTH/2, 90, 13)];
    tipNeedsConsumeLabel.textColor = [UIColor colorWithRed:172/255.0 green:168/255.0 blue:213/255.0 alpha:1.0];
    tipNeedsConsumeLabel.font = SMALLFONT_10;
    [tipView addSubview:tipNeedsConsumeLabel];
}
- (void)knownClick{
...
/*
 点击了身体状况
 */
- (void)bodyHealthClick{
}
- (void)helpClick{
    EnergyHelpViewController *controller = [[EnergyHelpViewController alloc] init];
...
{
    if(!_calendarView){
        _calendarView = [[CLWeeklyCalendarView alloc] initWithFrame:CGRectMake(0, SCREEN_WIDTH/2 + 30 + 45, SCREEN_WIDTH - 48, 49)];
...
- (void)chooseDairyClick{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        viewForTable.frame = CGRectMake(0 - SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT, SCREEN_WIDTH*2, SCREEN_HEIGHT - NAVIGATIONBAR_HEIGHT - TABBAR_HEIGHT);
    }];
    [chooseDairyButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    chooseDairyButton.backgroundColor = MAIN_COLOR_YELLOW;
    [chooseMineButton setTitleColor:MAIN_COLOR_YELLOW forState:UIControlStateNormal];
...
-(void)personalAltView{
    ...
    UILabel *titleLabel=[[UILabel alloc]initWithFrame:CGRectMake(0,0,nowView.frame.size.width , nowView.frame.size.height/5)];
    titleLabel.text=@"记录体重";
    titleLabel.textAlignment=NSTextAlignmentCenter;
    [nowView addSubview:titleLabel];
...
-(void)causeAltView{
    ...
    UILabel *titleLabel=[[UILabel alloc]initWithFrame:CGRectMake(15,10,causeView.frame.size.width/2 , causeView.frame.size.height/5)];
    titleLabel.text=@"目标体重";
    //titleLabel.textAlignment=NSTextAlignmentCenter;
    [causeView addSubview:titleLabel];
...
- (void)chooseMineClick{
    ...
    [self doneLoadingTableViewData];
    [UIView animateWithDuration:0.2 animations:^{
        viewForTable.frame = CGRectMake(0, NAVIGATIONBAR_HEIGHT, SCREEN_WIDTH*2, SCREEN_HEIGHT - NAVIGATIONBAR_HEIGHT - TABBAR_HEIGHT);
    }];
...
- (void)weightPress:(UITapGestureRecognizer*)gesture
{
    ...
    PNTestViewController *Vc=[[PNTestViewController alloc]init];
    Vc.goalWeight=targetWeightLabel.text;
...
//    NSArray *chatListArray = [[RCIM sharedRCIM]getConversationList:[NSArray arrayWithObjects:[NSNumber numberWithInt:ConversationType_PRIVATE],[NSNumber numberWithInt:ConversationType_GROUP], nil]];//只需要单聊的有未读消息
...
- (void)getPersonWeightInfo{
    ...
    [MyInfoRequest myWeightInfo:dic success:^(id response) {
        if ([[response objectForKey:@"state"] integerValue] == 1000) {
            ...
            //reducedWeightLabel.text = [NSString stringWithFormat:@"%@", [weightInfoDic objectForKey:@"haveweight"]];
            ...
            tipNeedsConsumeLabel.attributedText = str3;
        }
        else {
        }
    } failure:^(NSError *error) {
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component{
    return nowPicker.frame.size.width/2;
}
        ...
        if (indexPath.row == 0) {
            ...
            if (flagIndex == 1) {
                ...
                if (indexPath.row < trainArray.count+2) {
                    ...
                    [cell clickImage:^{
//                        CourseFunctionViewController *courseFunctionController = [[CourseFunctionViewController alloc]init];
//                        courseFunctionController.courseid = entity.courseid;
//                        courseFunctionController.hidesBottomBarWhenPushed = YES;
//                        [appDelegate.rootNavigationController pushViewController:courseFunctionController animated:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (currentIndex == 0) {
        if (indexPath.row == 0) {
            ...
            [appDelegate.rootNavigationController pushViewController:myInfoController animated:YES];
        }
    }
    else {
        if (indexPath.row < dairyArray.count) {
...
- (void)doneLoadingTableViewData{
    ...
    if (currentIndex == 0) {
        [self sortData];
        [myTableView reloadData];
        [_refresh egoRefreshScrollViewDataSourceDidFinishedLoading:myTableView];
    }
    else {
        [dairyTableView reloadData];
