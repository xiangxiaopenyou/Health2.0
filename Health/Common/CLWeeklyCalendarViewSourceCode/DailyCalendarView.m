...
{
    if(!_dateLabel){
        _dateLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.5, 0, DATE_LABEL_SIZE, DATE_LABEL_SIZE)];
...
//        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor colorWithHex:0x0081c1];
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor blackColor]: [UIColor yellowColor];
        
        //self.dateLabel.textColor = (blnSelected)?[UIColor colorWithHex:0x0081c1]:[UIColor whiteColor];
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithRed:230/255.0 green:187/255.0 blue:13/255.0 alpha:1.0]:[UIColor blackColor];
    }else{
